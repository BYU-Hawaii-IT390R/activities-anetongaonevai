...
def analyze_failed_logins(path: str, min_count: int):
    ...
    # regex assisted by ChatGPT: match failed login from IP
    FAILED_LOGIN_PATTERN = re.compile(r"Failed login attempt \[.*\] from (\d+\.\d+\.\d+\.\d+)")
    ip_counter = Counter()
    ...
    # Filter IPs with fewer than min_count
    filtered = Counter({ip: count for ip, count in ip_counter.items() if count >= min_count})
    _print_counter(filtered, "Failed login attempts per IP", "IP Address")
...
def analyze_successful_creds(path: str):
    ...
    print("Username     Password     Unique Source IPs")
    print("--------------------------------------------")
